...
public class SecurityConfig {
    ...
    public SecurityFilterChain prodSecurityFilterChain(HttpSecurity http) throws Exception {
        ...
        return http
                .authorizeHttpRequests(auth -> auth
                        .requestMatchers(BaseSecurityConfig.COMMON_ALLOWLIST).permitAll()
                        .requestMatchers(HttpMethod.GET, "/api/notifications/**").permitAll()               // 조회만 허용
                        .requestMatchers(HttpMethod.PATCH, "/api/notifications/**").permitAll()               // 조회만 허용
                        .requestMatchers("/api/notifications/**").hasAuthority(RoleType.ROLE_ADMIN.name())  // 변경은 관리자만
                        .anyRequest().authenticated()
                )
                .build();
    }
